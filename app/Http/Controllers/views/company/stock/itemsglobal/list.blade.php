    ...
    <div class="content">
        <div class="row">
            <div class="col-sm-6 col-md-3">
                <a class="block block-link-hover3 text-center" href="{{ route('itemsglobal.create', request()->route('profile')) }}">
                ...
                <table class="table table-borderless table-striped table-vcenter">
                    ...
                    <tbody>
                        @foreach ($itemsglobal as $item)
                            ...
                            @php
                            $itemMedias = $item->getMedia('images');
                            @endphp
                            <tr>
                                <td>
                                    @if ($item->getMedia('images')->first()!=null)
                                        <img src={{ asset($item->getMedia('images')->first()->getUrl('thumb')) }} />
                                    </td>
                                    @endif

                                    <td class="text-center">
                                        <a class="font-" href="{{ route('itemsglobal.edit',[ request()->route('profile'),$item]) }}">
                                            <strong>{{$item->sku}}</strong>
                                        </a>
                                    </td>
                                    <td class="visible-lg">
                                        <a href="{{ route('itemsglobal.edit',[ request()->route('profile'),$item]) }}">{{$item->name}}</a>
                                    </td>
